...
export class Parser {
    ...
    private get_type_from_string = (type: string): KITEventType => {
        return (
            KIT_ENDPOINT_EVENT_TYPES_MAP[type.toLowerCase()] ??
            KITEventType.Sonstiges
        );
    };

    private get_event_occurrences_by_elements = (
        event_occurrences_elements: Element[],
    ): KITEventOccurrence[] => {
        ...
        for (const element of event_occurrences_elements) {
            ...
            const date_occurrence_match = date_element?.textContent?.match(
                KIT_EVENT_DATE_OCCURRENCE_REGEX,
            );

            if (date_occurrence_match === null || time_element === null)
                continue;

            const [, week_day, event_occurrence] = date_occurrence_match;
            const date = event_occurrence.match(KIT_EVENT_FIXED_DATE_REGEX)
                ? event_occurrence
                : null;

            if (room_element !== null) {
                const room_name = room_element?.textContent;
                const room_gguid_match = room_element
                    .getAttribute("href")
                    ?.match(KIT_GGUID_GROUP_REGEX);
                const room_id = room_gguid_match
                    ? room_gguid_match[1]
                    : room_name;
    ...
    private get_event_from_raw_event_data = (
        event: RawEventDataItem,
    ): KITEvent | null => {
    ...
    private get_all_events_from_raw_data_items = (
        event: RawEventDataItem[],
    ): KITEvent[] => {
        return event
            .map((event) => this.get_event_from_raw_event_data(event))
    ...
    public get_all_events = (): KITEvent[] => {
        ...
        return this.get_all_events_from_raw_data_items(raw_events).filter(
